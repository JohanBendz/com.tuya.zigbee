...
class TuyaZigBeeLightDevice extends ZigBeeDevice {
    ...
    async changeDimLevel(dim, opts = {}) {
        ...
        if (this.hasCapability('light_mode') && this.getCapabilityValue('light_mode') === 'temperature') {
            ...
            return this.levelControlCluster.moveToLevel(moveToLevelWithOnOffCommand)
                .then(async result => {
                    // Update onoff value
                    if (dim === 0) {
                        await this.setCapabilityValue('onoff', false);
    ...
    async changeColorTemperature(temperature, opts = {}) {
        ...
        this.colorControlCluster.tuyaRgbMode({ enable: 0 });
    ...
    async changeColor({ hue, saturation, value }, opts = {}) {
        ...
        this.colorControlCluster.tuyaRgbMode({ enable: 1 });
    ...
    async onEndDeviceAnnounce() {
        ...
        if (typeof colorTemperatureMireds === 'number') {
            await this.setCapabilityValue('light_temperature',1-(colorTemperatureMireds / MAX_COLORTEMPERATURE));
        }
    }
