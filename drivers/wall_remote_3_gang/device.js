...
class wall_remote_3_gang extends ZigBeeDevice {
    ...
    async onNodeInit({zclNode}) {
        ...
        zclNode.endpoints[3].bind(CLUSTER.ON_OFF.NAME, new OnOffBoundCluster({
            onToggle: this._rightToggleCommandParser.bind(this),
            onSetOn: this._rightOnCommandParser.bind(this),
            onSetOff: this._rightOffCommandParser.bind(this),
            onWithTimedOff: this._rightOnWithTimedOffCommandParser.bind(this),
            offWithEffect: this._rightOffWithEffectCommandParser.bind(this)
        }));
    ...
    _leftToggleCommandParser() {
        ...
        .then(() => this.log('Wall Remote 3 Gang - button='+button))
        .catch(err => this.error('Error triggering Wall Remote 3 Gang button', err));
    }
    _middleToggleCommandParser() {
        let button = 'middle-oneClick';
        this._buttonPressedTriggerDevice.trigger(this, {}, { action: button })
        .then(() => this.log('Wall Remote 3 Gang - button='+button))
        .catch(err => this.error('Error triggering Wall Remote 3 Gang button', err));
    }
    ...
    _middleOnCommandParser() {
        let button = 'middle-twoClicks';
        this._buttonPressedTriggerDevice.trigger(this, {}, { action: button })
        .then(() => this.log('Wall Remote 3 Gang - button='+button))
        .catch(err => this.error('Error triggering Wall Remote 3 Gang button', err));
    }
    ...
    _middleOffCommandParser() {
        let button = 'middle-longPress';
        this._buttonPressedTriggerDevice.trigger(this, {}, { action: button })
        .then(() => this.log('Wall Remote 3 Gang - button='+button))
        .catch(err => this.error('Error triggering Wall Remote 3 Gang button', err));
    }
    ...
    _middleOnWithTimedOffCommandParser() {
        let button = 'middle-shortRelease';
        this._buttonPressedTriggerDevice.trigger(this, {}, { action: button })
        .then(() => this.log('Wall Remote 3 Gang - button='+button))
        .catch(err => this.error('Error triggering Wall Remote 3 Gang button', err));
    }
    ...
    _middleOffWithEffectCommandParser() {
        let button = 'middle-longRelease';
        this._buttonPressedTriggerDevice.trigger(this, {}, { action: button })
        .then(() => this.log('Wall Remote 3 Gang - button='+button))
        .catch(err => this.error('Error triggering Wall Remote 3 Gang button', err));
    }
