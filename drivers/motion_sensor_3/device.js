...
class motion_sensor_3 extends TuyaSpecificClusterDevice {
    ...
    async onNodeInit({ zclNode }) {
        ...
        this.batteryInterval = setInterval(async () => {
            try {
                await zclNode.endpoints[1].clusters.tuya.read({ dp: 4 });
                //await zclNode.endpoints[1].clusters.tuya.read({ dp: 5 });
            } catch (err) {
	...
	if (data.dp === 0) {
            ...
            this.log('Received battery percentage');
        } else if (data.dp === 5) {
            // Tampering switch activated
            this.setCapabilityValue('alarm_tamper', data.data.readUInt8(0) === 1);
            this.log('Tampering alarm triggered!');
        } else if (data.dp === 1) { 
            // Set the value of the 'alarm_motion' capability
            this.setCapabilityValue('alarm_motion', data.data.readUInt8(0) === 0);
            this.log('Motion alarm triggered!');
        } else {
            this.log('Received a response or report:', data);
        }
